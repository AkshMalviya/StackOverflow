  ...
  return (
	<nav className='main-nav'>
		...
		<form className={isActive ? 'drop-menu' : ''}>
			...
			<svg className='search-icon' width="18" height="18"  viewBox="0 0 18 18"><path  d="m18 16.5-5.14-5.18h-.35a7 7 0 1 0-1.19 1.19v.35L16.5 18l1.5-1.5ZM12 7A5 5 0 1 1 2 7a5 5 	0 0 1 10 0Z"/></svg>
			</div>
		</form>
		{ User === null ? 
		<Link to='/Auth' className='navlink login-btn'>Log in</Link>:
			<>
			<Link to={`/Users/${User?.result?._id}`} style={{textDecoration:"none"}} className='profile-logo'> <Avatar backgroundColor="blue" px="10px" py='7px' borderRadius="50%" color='white'>{User.result.name.charAt(0).toUpperCase()}</Avatar> </Link>
